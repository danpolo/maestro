...
* ``PROJECT_PATTERNS`` and the legacy-path check are scanned everywhere except the
  explicitly named build scaffolding below. Those files document *this build* — the
  design, the process, the live progress log and the overnight driver — and
  legitimately name the project the code is being extracted from. They are not part
  of the maestro deliverable. Everything else, including all of ``maestro/``,
  ``tests/`` and ``templates/``, stays strict, so a new file is always caught.
"""
import re
...
def _is_scaffold(path: Path) -> bool:
    rel = _rel(path)
    return rel in BUILD_SCAFFOLD or rel.startswith("docs/plans/")
