...
## 2026-09-07 — graph architecture plan complete; implementation not started

The complete eight-file local research packet was independently reassessed against the current
source and documentation. The resulting single proposal is the
[graph architecture and implementation plan](graph-engineering/MAESTRO_GRAPH_ENGINEERING_ARCHITECTURE.html),
with a separate [18-item research blind-spots record](../GRAPH_ENGINEERING_RESEARCH_BLIND_SPOTS.md).
It defines one durable workflow runtime, project/task construction, distinct configured agent roles,
paid/free/hybrid policies, exact model/effort routing, and P0–P12 implementation acceptance criteria.
No additional external research, runtime implementation, service change or deployment was performed.

Verification: 30 HTML links resolve locally; 13 implementation phases and four figures are present;
offline desktop/mobile rendering, dark mode, navigation and enlargement were checked; no external
assets or browser errors were observed. `git diff --check` passed. Existing watchdog/test edits and
the two earlier handoffs were preserved. Both deliverables were sent to Dan's private inbox.
The research directory, including the HTML plan, remains intentionally gitignored.

**Next proposed coding task, when implementation is authorized:** P0's isolated baseline fixtures
and measurement record, owned by a fresh Codex session. Start from the
[launchable P0 handoff](../handoffs/2026-09-07-graph-engineering-p0.md).
The existing readiness caveats above remain open; this documentation work does not close them.

## 2026-09-08 — local graph-engineering blind spots exercised

Completed the machine-resolvable experiment pass authorized from the graph blind-spots record. The
work added a validated evidence schema, a fixed six-class task catalog, six validation-dependency
mutations, bounded process/SQLite/storage/action fixtures, a local audit runner, and a two-project
init rehearsal. It made two narrow production corrections supported by failing tests: `init` no
longer invents a test command from package metadata alone, and generated service units now follow
the operator-required `Type=forking` plus direct named-TMUX ownership contract.

Measured outcomes and limitations are recorded in
[`GRAPH_ENGINEERING_RESEARCH_BLIND_SPOTS.md`](../GRAPH_ENGINEERING_RESEARCH_BLIND_SPOTS.md) and the
three JSON artifacts under `artifacts/graph-engineering/`. Highlights: disposable Python and Node
projects both completed init, the supervised idle loop, and their generated test adapters; the
policy fixture found three known misses and zero false requirements across eight cases; all six
seeded validation dependencies were caught by the full fixture suite; the unprivileged `bwrap`
profile blocked parent/child escape writes and masked one credential directory while deliberately
leaving shared Git metadata read-only; and the receipt fixture reconciled one effect while blind
replay produced two.

One useful read-only recovery review was run on Claude Code 2.1.263 / Sonnet 4.6 / low and
Antigravity 1.1.27 / Gemini 3.8 Flash / low. The stored manual classifications map each response to
the five pre-fixed defects and 17/17 weighted points, but the raw responses and exact invocations
were not retained, so these scores remain provisional. Codex 0.153.4 / configured GPT-5.6 Sol /
high was not run because the pre-sample showed 84% five-hour and 82% weekly usage. The stored
pre/post signals proved account attribution is not reliable enough for route economics, and two
outcomes on one task do not establish a quality floor. No external research, free/open model, paid
API, real project/service mutation, or privileged operation was performed.

Verification: `tests/graph_engineering` passed **42 tests**; both backend-free init rehearsals and
the refreshed two-run local audit exited 0 with every compared stable field equal; the complete
repository gate passed **3438 tests with 1 expected xfail in 159.28s**; `git diff --check` passed
before the final documentation update.

## 2026-09-08 — third-party verification gate (B06/B17) implemented

The third category of graph-engineering blind spots — implementation-time external verification —
was closed as a *mechanism* rather than as an answer. B06 (backend capabilities) and B17 (free-use
guarantees) stay **open** in the blind-spots register on purpose: both are facts about one exact
version, and the local pass had already caught the installed CLIs having left the supplied research
behind on three separate points. Researching them now buys a stale answer; the useful artefact is a
gate that fires at adoption time.

Added:

- `maestro/thirdparty.py` — the register reader, schema validation and drift comparison. A row that
  names a `verified_version` must cite its evidence; a dependency row must name a licence *and where
  it was read from*; a backend row must list the controls actually observed. Parsing raises rather
  than degrading, because a register that degrades to "nothing to check" is a gate that passes
  everything.
- `maestro/thirdparty.json` — nine rows: `claude` 2.1.263, `codex` 0.153.4 (effort is a `--config`
  key on this version, not a flag), `agy` 1.1.27 as a candidate backend, the five declared Python
  distributions with licences read from installed metadata, and Tree-sitter as an unverified
  candidate so that enabling it trips the gate.
- `artifacts/thirdparty/python-dependency-licences.json` — the retained licence observation.
- `maestro doctor --thirdparty` — the gate itself, exit non-zero on an unverified or drifted subject
  that is actually in use. The same check runs inside the full `doctor` as a WARN, so an agent CLI
  that auto-updates overnight is loud rather than fatal.
- `docs/DESIGN.md` §15 — the Definition-of-Done rule and what the register deliberately is *not*
  (a verification ledger, not P6's runtime capability catalog).

Verification: `pytest -q` unfiltered → **3462 passed, 1 xfailed, exit 0** (2026-09-08).
`maestro doctor --thirdparty` → `[OK] thirdparty: 9 subject(s): 1 candidate, 8 verified`, exit 0.
New tests: `tests/test_thirdparty.py` (20) plus four `doctor` cases in `tests/test_cli.py`.

Not done here: no adapter was redesigned, no dependency was adopted, and the architecture HTML was
deliberately left unedited — the proposed plan changes are recorded separately for review.

## 2026-09-08 — graph plan revision 2: one source of truth

`docs/graph-engineering/MAESTRO_GRAPH_ENGINEERING_ARCHITECTURE.html` is now **the single source of
truth for the plan**. Everything that had accumulated in review documents, evidence artifacts and
conversation is folded into it, and it carries an amendment log recording what changed and on what
evidence. `REVIEW_RESEARCH_VS_PLAN.md` is marked superseded; the blind-spots register is explicitly
demoted to open *evidence*, never plan content.

Folded in (28 in-place amendments):

- **P6** — grade the *acquisition path* (pull vs push), not the CLI: all three backends expose quota,
  and Antigravity carries four independent pools. `QuotaObservation` gains `observed_at` vs
  `written_at`, a staleness bound and `carried_forward`; an unchanged reading is never a zero delta.
  An isolation contract plus a zero-work control gate attribution. `effort_delivery` joins
  `effort_support`. Outcome statistics gain `(workflow_template_id, template_version,
  active_fragment_set)`. Version facts moved out of the prose into `maestro/thirdparty.json`.
- **P5** — the library becomes templates *and* named versioned fragments; the compiler's invariant is
  the minimal shape that discharges the required outcomes; selection is deterministic and a planner
  may only propose. Setup emits `unresolved[]` facts that fail closed.
- **P7** — an enforced profile must *permit* the role's required writes; implementer branches must be
  namespaced (`impl/<task>`) before that phase.
- **P1/P3, P4, P9, P11** — the writer lease binds to the writing process, not the launcher; adapters
  declare `effect_class` and a probe; each parser and grammar needs its own verification row;
  per-project pinning needs `bootstrap.reexec_into_adopted_version` to read its repo from argv.
- **D04, D05, guardrails, free-use boundary** — the integration verification gate becomes a standing
  rule for every phase; free mode is an API free tier reached later, reusing P6's quota contract.
- **E1/E2** — fixtures qualify as gates only via a recorded calibration run; shapes are promoted and
  demoted symmetrically with a shadow cold start; E2 runs on Maestro's own repositories.
- **Withdrawn** — the `init` exit-code defect (does not reproduce on the current tree) and the model
  residency proposal (followed from a local-serving assumption the operator ruled out).

Verification: page re-rendered offline — 16 sections, 12 tables, no broken TOC anchors, no console
errors, no horizontal overflow. `pytest -q tests/test_purity.py tests/test_thirdparty.py
tests/test_no_name_branching.py tests/test_templates.py` → 84 passed.
`maestro doctor --thirdparty` → exit 0.

## 2026-09-08 — pilot project selection and safeguards recorded

Operator direction: designate the concrete pilot targets across the graph engineering evolution plan.

1. **Phase P0 (Greenfield Scratch Baseline):** **DuetFlow** (`/home/dan/projects/duetflow`, based on `~/projects/project-ideas/DuetFlow-README.md`). A headless Python service with SQLite, pure algorithmic scoring, 50/50 playlist balancing, and sub-second deterministic `pytest` tests. Satisfies P0's scratch setup rehearsal and single real coding task under baseline instrumentation to close B01's first half.
2. **Phase P12 (Brownfield Multi-Project Qualification):** **instagram-to-value** (`/home/dan/projects/instagram-to-value`), paired alongside DuetFlow. A living Python pipeline with 235 passing unit tests in 1.44s and an active backlog in `PLAN.md`.
3. **Operational Safeguards for instagram-to-value:**
   - **Daemon Protection (`bot_files`):** Set `bot_files: ["scripts/bot.py", "scripts/worker.py"]` in `project.yaml`. Because `itv-bot` and `itv-worker` run live in background tmux sessions, touching these entrypoints halts resumable auto-merge and requires Dan's explicit review.
   - **Queue & Media Confinement:** Add `jobs/`, `staging/`, `media/`, `out/`, `logs/`, and `config/` to `deny_list_extra` in `project.yaml`. Implementers in worktrees are forbidden from mutating live queues or staged proposals.
   - **Deterministic Gate:** Set `gate.chain: [test]` to run the mock-isolated 1.44s `pytest -q` suite, avoiding external network calls or heavy ASR/OCR model runs during worktree verification.
   - **TMUX Isolation:** Daemon sessions (`itv-bot`, `itv-worker`) remain untouched; Maestro implementers run strictly within the `agents` session.

Recorded in `docs/graph-engineering/specs/phases/P00_MIGRATION_BASELINE.md`, `P12_QUALIFICATION_AND_RELEASE.md`, `handoffs/2026-09-07-graph-engineering-p0.md`, `GRAPH_ENGINEERING_RESEARCH_BLIND_SPOTS.md`, and `docs/graph-engineering/MAESTRO_GRAPH_ENGINEERING_ARCHITECTURE.html`.

## 2026-09-09 — grill-with-docs → maestro init handoff contract

Operator note ahead of the P0 implementation session: every project, including a "fresh new" one,
is grilled with the `grill-with-docs` skill in a separate session first. Maestro's fresh-project
onboarding therefore always starts from that skill's output (`CONTEXT.md`, `docs/adr/`, and usually
`docs/PLAN.md`), never from a blank interview. Reviewed DuetFlow's actual grill output against
`maestro init` / the `maestro-setup` skill.

**Structural findings (maestro side, not yet fixed in code):**

1. **The design documents never reach the implementer.** `maestro/implementer.py:350` and `:419`
   build every task brief telling the agent to read `docs/ROADMAP.md` and `docs/PROJECT.md` —
   they never name `CONTEXT.md` or `docs/adr/`. On a grilled project every autonomous task starts
   blind to the canonical vocabulary and to every recorded decision.
2. **Maestro has no concept of an ADR.** Nothing in `operating_preamble.md` or the gate stops an
   autonomous task from quietly reversing a recorded decision.
3. **The glossary transform is lossy.** `maestro/docs/upcoming.py:114` (`_GLOSSARY_BULLET`) requires
   `- **Term** — definition` on one line; a `CONTEXT.md` entry is a bolded term, a colon, a
   multi-line definition and an `_Avoid_:` list. The banned synonyms are dropped entirely —
   `_term_aliases` (`:117`) derives aliases only from a parenthetical.
4. **Acceptance criteria have nowhere to go.** `maestro/docs/roadmap.py` parses no acceptance key,
   so a build phase's "Done when" can only survive inside free-text `notes`.

Findings 1 and 2 are worked around per-project by the setup skill (below). 3 and 4 would need
maestro code changes and are **not** scheduled.

**Two silent failures that will hit the DuetFlow P0 pilot — fix before `maestro init`:**

- **No root manifest.** `/home/dan/projects/duetflow` has `tests/` and `.venv` but no
  `pyproject.toml`, `requirements.txt` or `package.json`. `_derive_facts` (`maestro/cli.py:186`)
  detects a test command only from those three plus `go.mod`, so `test_command` renders empty and
  the generated `adapters/test` exits 1 with a TODO. Honest, but the gate then certifies nothing
  and every task parks at the merge gate. Add a manifest, or wire `adapters/test` by hand.
- **No git remote.** `git remote get-url origin` is empty in duetflow. `maestro/merge.py:558`
  and `:640` run `git push origin main` without checking the return code, so merges land locally
  and silently never leave the machine.

**Changed (outside this repo):** `~/.claude/skills/maestro-setup/SKILL.md`, 179 → 354 lines. New
Step 2 harvests the grill output and turns the interview into a confirmation pass; Step 2a spells
out the glossary transform against the real regex; Step 2b requires a "Source documents" section at
the top of `docs/PROJECT.md` (the only path from a task brief to `CONTEXT.md` and the ADRs); Step 5a
converts build phases to task blocks carrying "Done when" verbatim into `notes`; Step 6 item 5
appends an ADR-precedence clause to the rendered `operating_preamble.md`; Step 1/Step 7 add
pre-flight checks and reporting for the two silent failures above.

**Also changed (outside this repo), approved by Dan the same day:**
`~/.claude/skills/grill-with-docs/SKILL.md` plus a new `PLAN-FORMAT.md` reference file.
`docs/PLAN.md` is now a contracted third output of that skill alongside `CONTEXT.md` and
`docs/adr/` — previously it was written only by chance, which is why DuetFlow has one and a future
grilled project might not. Build phases now carry a stable `NN-slug` id that is never renumbered, an
explicit `Depends on` listing predecessor ids rather than prose ordering, an observable "Done when"
naming the invariants the phase owns, and a `Who` column of `agent` or `operator`. The skill's
closing step asks the user directly which phases only they can do — an interactive login, a
credential they hold, a privileged install — because an OAuth flow and a pure function look
identical in prose and nothing downstream can infer the difference.

`maestro-setup` additionally now states that it may ask the operator whenever it is missing data or
reading something two ways, via `AskUserQuestion`, rather than inferring or leaving a scaffold
default: it runs in an interactive session precisely because `init`/`doctor` already cover
everything derivable. Harvesting is inference, and a confidently wrong harvested answer is worse
