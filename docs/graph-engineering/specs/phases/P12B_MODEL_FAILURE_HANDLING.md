...
- [ ] **F7: Escalation to a stronger diagnoser** (`compiler`, `runner`, `routing`):
  ...
  - **Inputs.** `diagnose` is an agent node with role `diagnoser`. Its inputs are the failing node's spec, its brief, every attempt's failure record and evidence, and the verification output.
    - They reach the model as a section of the brief it is given (`worker.agent_brief`, rendered by `workflows/failure_context.py`), not through the context manifest, which is a pure function of the task and drives routing and statistics. The gate worker reports each check's stdout/stderr, and the controller stores it as a `verification_output` artifact at ingest.
    - The spec, the brief, every binding and every failure record go in **full**. Only the verification output is bounded: an aggregate cap of `EVIDENCE_TOKEN_CAP` = **16,000 tokens** (initial value, to be tuned), measured with `repository.context.estimate_tokens`. Failed checks are spent on first. A long output keeps its head and tail, with explicit `[EXCERPT …]` / `[… N characters omitted …]` / `[OMITTED …]` markers.
    - Every stored `failure_context` artifact carries `sizes: {full_chars, delivered_chars, evidence_cap_tokens}`, so the cap can be re-evaluated against real diagnoses.
